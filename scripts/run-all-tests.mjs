...
for (const scriptName of allTestScripts) {
  const scriptStartedAt = Date.now();
  await runScript(scriptName);
  console.log(`[test:all] ${scriptName} completed in ${formatDuration(Date.now() - scriptStartedAt)}`);
}

console.log(`[test:all] total time: ${formatDuration(Date.now() - startedAt)}`);
