...
for (const scriptName of e2eProdTestScripts) {
  const scriptStartedAt = Date.now();
  await runScript(scriptName);
  console.log(
    `[test:e2e:prod:all] ${scriptName} completed in ${formatDuration(Date.now() - scriptStartedAt)}`,
  );
}

console.log(`[test:e2e:prod:all] total time: ${formatDuration(Date.now() - startedAt)}`);
