import {
  ...
  workspaceModuleRoleListResponseSchema,
...
export function createAdminWorkspaceModuleLabRolesRouteHandlers({
  ...
  async function GET(_request, context) {
    ...
    const upstream = await fetchImplementation(
      `${getInternalApiUrl()}/v1/admin/workspaces/${encodeURIComponent(workspaceId)}/module-roles/module-lab`,
      {
        method: "GET",
        headers: {
      ...
      return jsonResponse(
        parsedError.success
          ...
          : {
              ...
              message: "The admin workspace module role list request failed.",
            },
        { status },
      );
    }

    const parsedResponse = workspaceModuleRoleListResponseSchema.safeParse(payload);

    if (!parsedResponse.success) {
      return jsonResponse(
        {
          error: "admin_workspace_module_role_list_failed",
          message: "The admin workspace module role list response was invalid.",
        },
        { status: 502 },
      );
    }
